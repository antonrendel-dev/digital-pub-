...
1. Cron на красном сервере (<proxy-host>) запускает sync-скрипт
...
- **Sync остаётся на красном сервере** (<proxy-host>), не переносится на NetAngels. Меняется только ORM: Prisma → Payload Local API. Причина: sync уже работает как отдельный процесс на красном сервере — не трогаем инфраструктуру, устраняем только зависимость от Prisma. `setImmediate` на NetAngels ненадёжен из-за Passenger-окружения.

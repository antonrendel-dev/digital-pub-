...
describe('раздел профессий', () => {
  ...
  it('профессия попадает в раздел только с живым ресурсом вакансий', () => {
    // Страница-обещание без вакансий хуже её отсутствия — это soft-404. Ровно
    // на этом отсеялись графический дизайнер и ретушёр.
    //
    // Порог снижен с 15 до 10 (31.08.2026) вместе со сменой единицы измерения.
    // Раньше вакансии профессии считались подстрокой по всему объявлению и
    // включали упоминания в требованиях: у видеомонтажёра выходило 126 при 55
    // настоящих. Теперь роль ищется в заголовке объявления, и прежние 15
    // «упоминаний» — это примерно 7 живых вакансий. Отсеянные профессии порог
    // проходить не начали: по новому счёту у графического дизайнера 2, у
    // ретушёра и веб-аналитика 0.
    const thin = professions
      .filter((p) => p.vacanciesAtMeasure < 10)

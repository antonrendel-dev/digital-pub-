...
export default function Footer() {
  return (
    <footer className="mt-10 bg-bg-card border-t border-border transition-colors duration-200">
      <div className="max-w-wrap mx-auto px-4 py-10">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          {/* Column 1: Brand */}
          <div>
            <div className="flex items-center gap-2.5 mb-2">
              <Image src="/logo.png" alt="Диджитал Паб" width={36} height={36} className="rounded-lg" />
              <span className="text-lg font-bold text-text tracking-tight">
                диджитал<em className="not-italic text-brand-yellow">паб</em>
              </span>
            </div>
            <p className="text-sm font-semibold text-text mt-2">Место, где встречаются хорошие люди</p>
            <div className="mt-2 pt-2 border-t border-border-light">
              <p className="text-[13px] text-text-muted leading-relaxed">Вакансии и резюме из Telegram-каналов.</p>
              <p className="text-[13px] text-text-muted leading-relaxed">Находи быстро, откликайся легко.</p>
            </div>
          </div>

          {/* Column 2: Navigation */}
          <div>
            <div className="text-sm font-bold text-text uppercase tracking-wide mb-3">Навигация</div>
            <div className="space-y-2">
              <Link href="/vacancies" className="block text-sm text-text-muted no-underline hover:text-accent transition-colors">Вакансии</Link>
              <Link href="/resumes" className="block text-sm text-text-muted no-underline hover:text-accent transition-colors">Резюме</Link>
              <Link href="/articles" className="block text-sm text-text-muted no-underline hover:text-accent transition-colors">Статьи</Link>
            </div>
          </div>

          {/* Column 3: Social */}
          <div>
            <div className="text-sm font-bold text-text uppercase tracking-wide mb-3">Мы в соцсетях</div>
          ...
          <div>
            <div className="text-sm font-bold text-text uppercase tracking-wide mb-3">Работодателям</div>
            <div className="space-y-2">
              <a href="https://t.me/resume_vac_bot" target="_blank" rel="noopener noreferrer" className="block text-sm text-text-muted no-underline hover:text-accent transition-colors">
                Разместить вакансию
              </a>
              <a href="https://t.me/resume_vac_bot" target="_blank" rel="noopener noreferrer" className="block text-sm text-text-muted no-underline hover:text-accent transition-colors">
                Реклама
      ...
      <div className="border-t border-border-light py-4">
        <div className="max-w-wrap mx-auto px-4 flex flex-col sm:flex-row items-center justify-between text-xs text-text-light gap-2">

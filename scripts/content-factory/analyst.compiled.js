...
function getPublishedArticleTitles() {
  ...
  return fs2.readdirSync(ARTICLES_DIR).filter((f) => f.endsWith(".mdx")).flatMap((f) => {
...
function getAllPlannedTopics() {
  ...
  return fs2.readdirSync(DATA_DIR).filter((f) => f.startsWith("topics_") && f.endsWith(".json")).flatMap((f) => {
...
function currentQueue() {
  ...
  const files = fs2.readdirSync(DATA_DIR).filter((f) => f.startsWith("topics_") && f.endsWith(".json")).sort();

...
const SCRIPTS_DIR = path_1.default.dirname(new URL(import.meta.url).pathname)
const PROJECT_ROOT = path_1.default.resolve(SCRIPTS_DIR, '../..')
const CONTENT_DIR = path_1.default.join(PROJECT_ROOT, 'content/articles')
...
const REFERENCE_IMAGE = path_1.default.join(SCRIPTS_DIR, 'reference.webp')
const PERSPECTIVES = [
...
async function main() {
  const mdxPath = path_1.default.join(CONTENT_DIR, `${slug}.mdx`)
  if (!fs_1.default.existsSync(mdxPath)) {
    console.error(`[regen] MDX не найден: ${mdxPath}`)
    process.exit(1)
  }
  const mdxContent = fs_1.default.readFileSync(mdxPath, 'utf-8')
  ...
  const destPath = path_1.default.join(IMAGES_DIR, destFilename)
  try {
    ...
    fs_1.default.copyFileSync(newImagePath, destPath)
  }
  ...
  fs_1.default.writeFileSync(mdxPath, updatedMdx)
  // Git add + commit + push
  ...
  await new Promise((resolve, reject) => {
    const child = (0, child_process_1.spawn)(
      'git',
      ['commit', '-m', `fix(image): regenerate cover for ${slug}`],
      {
        cwd: PROJECT_ROOT,
        stdio: 'inherit',
      }
  ...
  await new Promise((resolve, reject) => {
    const child = (0, child_process_1.spawn)('git', ['push', 'origin', 'main'], {
      cwd: PROJECT_ROOT,
      stdio: 'inherit',
    })
    child.on('close', (code) =>

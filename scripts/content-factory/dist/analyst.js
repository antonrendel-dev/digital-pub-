...
const ARTICLES_DIR = path_1.default.join(import.meta.dirname, '../../content/articles')
function getPublishedArticleTitles() {
  if (!fs_1.default.existsSync(ARTICLES_DIR)) return []
  return fs_1.default
    .readdirSync(ARTICLES_DIR)
    .filter((f) => f.endsWith('.mdx'))
    .flatMap((f) => {
      const raw = fs_1.default.readFileSync(path_1.default.join(ARTICLES_DIR, f), 'utf-8')
      const m = raw.match(/^title:\s*["']?(.+?)["']?\s*$/m)
      return m ? [m[1]] : []
    })
}
function getAllPlannedTopics() {
  if (!fs_1.default.existsSync(DATA_DIR)) return []
  return fs_1.default
    .readdirSync(DATA_DIR)
    .filter((f) => f.startsWith('topics_') && f.endsWith('.json'))
    .flatMap((f) => {
      const { topics } = JSON.parse(
        fs_1.default.readFileSync(path_1.default.join(DATA_DIR, f), 'utf-8')
      )
...
async function generateTopics() {
  ...
  const opportunities = await (0, yandex_js_1.fetchWebmasterOpportunities)(20)
...
[
  ...
  await Promise.all(
    topics.map(async (t) => {
      t.wordstatVolume = await (0, yandex_js_1.fetchWordstatVolume)(t.keyword)
    })
  )
...
async function main() {
  ...
  const filePath = path_1.default.join(DATA_DIR, `topics_${date}.json`)
  fs_1.default.writeFileSync(filePath, JSON.stringify({ date, topics }, null, 2))
  ...
  await (0, telegram_js_1.sendMessage)(msg)

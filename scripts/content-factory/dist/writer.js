...
const yandex_js_1 = require('./lib/yandex.js')
const DATA_DIR = path_1.default.join(import.meta.dirname, 'data')
const PROJECT_ROOT = path_1.default.resolve(import.meta.dirname, '..', '..')
const ARTICLES_DIR = path_1.default.join(PROJECT_ROOT, 'content', 'articles')
const IMAGES_DIR = path_1.default.join(PROJECT_ROOT, 'public', 'images', 'posts')
const SITE_URL = process.env.NEXT_PUBLIC_SERVER_URL || 'https://d-pub.ru'
const CODEX_BIN = path_1.default.join(os_1.default.homedir(), '.npm-global', 'bin', 'codex')
const CODEX_HOME = path_1.default.join(os_1.default.homedir(), '.codex')
const REFERENCE_IMAGE = path_1.default.join(import.meta.dirname, 'reference.webp')
const PERSPECTIVES = [
...
function snapshotGeneratedImages() {
  const generatedDir = path_1.default.join(CODEX_HOME, 'generated_images')
  const images = new Set()
  if (!fs_1.default.existsSync(generatedDir)) return images
  for (const session of fs_1.default.readdirSync(generatedDir)) {
    const sessionDir = path_1.default.join(generatedDir, session)
    try {
      for (const file of fs_1.default.readdirSync(sessionDir)) {
        if (file.endsWith('.png') || file.endsWith('.webp') || file.endsWith('.jpg')) {
          images.add(path_1.default.join(sessionDir, file))
        }
      }
    } catch {}
  }
...
function convertToWebP(srcPng, destWebp) {
  const script = `
    import('${path_1.default.join(PROJECT_ROOT, 'node_modules', 'sharp', 'lib', 'index.js')}')
      .then(m => m.default('${srcPng}').resize(900, 450, {fit:'cover'}).webp({quality:85}).toFile('${destWebp}'))
      .then(() => process.exit(0))
      .catch(e => { console.error(e.message); process.exit(1); })
  `
  ;(0, child_process_1.execSync)(`node --input-type=module`, {
    input: script,
    cwd: PROJECT_ROOT,
    timeout: 30000,
...
async function generateImageWithCodex(imagePrompt, slug, topicId) {
  if (!fs_1.default.existsSync(CODEX_BIN)) {
  ...
  const refArg = fs_1.default.existsSync(REFERENCE_IMAGE) ? ['-i', REFERENCE_IMAGE] : []
  const runCodex = () =>
    new Promise((resolve) => {
  ...
  const destWebp = path_1.default.join(IMAGES_DIR, `${slug}.webp`)
  try {
    ...
    const destPng = path_1.default.join(IMAGES_DIR, `${slug}.png`)
    fs_1.default.copyFileSync(newImage, destPng)
...
async function generateMdxArticle(topic) {
  ...
  const wordstatRaw = await (0, yandex_js_1.fetchWordstatKeywords)(topic.keyword)
...
- FAN-OUT: каждый H2 раскрывает РАЗНЫЙ аспект — не повторяй один угол в двух блоках
- АТРИБУТЫ: для статей о профессии план обязан покрыть ≥6 из: зарплата, навыки, образование, регионы, карьерный рост, инструменты, кому подходит, перспективы
- H2 КАК ВОПРОС: формулируй каждый H2 как суб-вопрос читателя («Сколько зарабатывает X?», «Какие навыки нужны?»)
- PLURAL SYMMETRY: чередуй ед. и мн. число в заголовках (не все H2 в одной форме числа)
- DECISION SUPPORT: если тема — профессия или карьера, добавь H2 «Кому эта профессия не подойдёт» (перед FAQ)
...
LEAD FORMULA: первые 3 предложения статьи строго по формуле:
  — Предложение 1: ≤12 слов, называет тему и главную сущность
  — Предложение 2: ≤18 слов, конкретная цифра + источник (hh.ru, SuperJob, Росстат)
  — Предложение 3: ≤15 слов, главная выгода или вывод для читателя

ATOMIC ANSWER: первое предложение каждого H2 = прямой ответ на вопрос блока, самодостаточный без контекста.

BERT-ЯНДЕКС: ключевое слово "${topic.keyword}" буквально (не перефразируя) — в первых 60 словах статьи.

BANNED: запрещены без исключений:
  ...
  — Метазачины абзацев: «По запросу X пользователи ищут...», «Многие задаются вопросом...»

ТАБЛИЦЫ: минимум 1 таблица на каждые 1500 слов (сравнение уровней / зарплат по регионам / навыков).

CONFIDENCE — три типа утверждений, не смешивать:
  ① Верифицированный факт — обязательно источник: «по данным hh.ru (2025)»
...
ЖИВОСТЬ: внутри абзаца используй синонимы и местоимения 1-3 раза — не повторяй главный ключ в каждом предложении.
...
- FAQ секция в конце: формат "### Вопрос?" → ответ 2-3 предложения, самодостаточный, не более 8 вопросов
...
6. Объём финального текста: 1200-1600 слов
...
Верни ТОЛЬКО финальный Markdown — без пояснений и комментариев.`)
  ...
  const allBiases = JSON.parse(
    fs_1.default.readFileSync(path_1.default.join(DATA_DIR, 'nudge-biases.json'), 'utf-8')
  ).biases
...
function getLatestTopicsFile() {
  ...
  return path_1.default.join(DATA_DIR, files[0])
}
function markTopicPublished(topicsFile, topicId) {
  const raw = JSON.parse(fs_1.default.readFileSync(topicsFile, 'utf-8'))
  const topic = raw.topics.find((t) => t.id === topicId)
  if (topic) topic.published = true
  fs_1.default.writeFileSync(topicsFile, JSON.stringify(raw, null, 2))
}
function gitCommitAndPush(slug, title, hasImage) {
  const mdxPath = path_1.default.join('content', 'articles', `${slug}.mdx`)
  ;(0, child_process_1.execSync)(`git add "${mdxPath}"`, { cwd: PROJECT_ROOT, stdio: 'inherit' })
  if (hasImage) {
    ;(0, child_process_1.execSync)(`git add "public/images/posts/${slug}.*" 2>/dev/null || true`, {
      cwd: PROJECT_ROOT,
      shell: '/bin/bash',
    })
  }
  const message = `feat: add article "${title}"`
  ;(0, child_process_1.execSync)(`git commit -m ${JSON.stringify(message)}`, {
    cwd: PROJECT_ROOT,
    stdio: 'inherit',
  })
  ;(0, child_process_1.execSync)('git push', { cwd: PROJECT_ROOT, stdio: 'inherit' })
}
// ─── Main ─────────────────────────────────────────────────────────────────────
async function main() {
  ...
  const { topics } = JSON.parse(fs_1.default.readFileSync(topicsFile, 'utf8'))
  ...
  const mdxPath = path_1.default.join(ARTICLES_DIR, `${result.slug}.mdx`)
  if (fs_1.default.existsSync(mdxPath)) {
  ...
  fs_1.default.mkdirSync(ARTICLES_DIR, { recursive: true })
  fs_1.default.writeFileSync(path_1.default.join(ARTICLES_DIR, `${result.slug}.mdx`), mdxContent)
  ...
  try {
    ...
    await (0, telegram_js_1.sendMessage)(`⚠️ Статья написана, но git push не удался:\n${e.message}`)
    process.exit(1)
  }
...
main().catch((e) => {
  ...
  ;(0, telegram_js_1.sendMessage)(`❌ Ошибка при генерации статьи:\n${e.message}`).catch(() => {})
  process.exit(1)
})

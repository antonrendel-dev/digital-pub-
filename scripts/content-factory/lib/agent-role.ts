...
/**
 * Срезает метку роли, с которой профиль велит начинать каждый ответ:
 * `~/.claude/agents/writer.md` — «Каждый ответ начинаешь с метки [WRITER]»,
 * analyst.md — то же с [ANALYST].
 *
 * В чате метка полезна: видно, кто из агентов говорит. В заводе ответ модели
 * уезжает прямо в артефакт, и метка ушла на сайт — 7 статей за август вышли
 * с `[WRITER]` перед текстом, ещё одна — с `[ANALYST]` внутри alt графика.
 * Профили при этом трогать нельзя: они общие с чатом.
 *
 * Режем только в начале строки и только известные роли — иначе пострадает
 * легитимная ссылка вида `[SMM](/vacancies/smm)` в середине абзаца.
 */
const ROLE_TAG_RE = /^\s*\[(?:WRITER|ANALYST|SEO|EDITOR|MARKETER|REVIEWER)\]\s*/

export function stripRoleTag(text: string): string {

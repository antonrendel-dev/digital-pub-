...
// Образец голоса вместо счётных норм. Модели лучше подражают тексту, чем
// выполняют «1–2 абзаца из одного предложения»: нормы дали 11 афоризмов
// в одной статье и одну фразу редакции в 13 файлах / 16 вхождений
// (аудит 04.09.2026, срез content/articles на эту дату).
const VOICE_SAMPLE_FILE = path.join(import.meta.dirname, 'assets', 'voice-sample.md')

function loadVoiceSample(): string {
  try {
    return fs.readFileSync(VOICE_SAMPLE_FILE, 'utf-8').trim()
  } catch {
    console.warn(`[writer] образец голоса не найден: ${VOICE_SAMPLE_FILE} — пишу без него`)
    return ''
  }
}
...
Верни JSON строго такого вида, без пояснений:
{
  ...
  "exactPhrases": [{"phrase": "фраза (в тексте — в любой грамматической форме)", "uses": 2}],
...
- exactPhrases: только фразы, которые НЕ содержат главный ключ целиком. Вордстат
  ...
  вхождение тратит бюджет главного ключа, а шесть обязательных мест его
  уже выбирают. Такие фразы идут в dilutedPhrases, не сюда. Суммарно не больше 12
  обязательных вхождений на статью. Приёмка считает их по основам слов: писатель
  склоняет фразу и меняет порядок слов, форму поисковой строки не копирует.
...
- bertAnchors: 3-5 слов из поискового запроса, которые обязаны присутствовать во всех четырёх BERT-полях Яндекса (Title, H1+H2, первые 60 слов, meta description) — в любой грамматической форме, но не синонимами (Яндекс лемматизирует, «резюме таргетолога» и «резюме для таргетолога» — одно). Берутся из самого частотного кластера Wordstat.
...
Выдай JSON (без лишнего текста):
{
  ...
  const dynamicSeoBlock = [
    (seoData.bertAnchors || []).length
      ? `BERT-ЯКОРЯ ЯНДЕКСА (4.1b): слова "${seoData.bertAnchors!.join('", "')}" должны присутствовать во ВСЕХ четырёх BERT-полях: Title, H1/H2-заголовках, первых 60 словах статьи, meta description. Не синонимами, но в любой грамматической форме — склоняй как в живой речи. Не выделяй их жирным.`
      : '',
    (seoData.bm25Context || []).length
      ? `BM25TP-КОНТЕКСТ (2.2b): в радиусе ±10 слов от каждого вхождения ключа "${topic.keyword}" должны стоять тематические слова из этого списка: ${seoData.bm25Context!.join(', ')}. Яндекс оценивает семантическое окружение — эти слова усиливают ранжирование.`
      : '',
    (seoData.antifakeMarkers || []).length
      ? `АНТИФЕЙК (AIO-resistant, 2.6c): обязательно включи в тело статьи опровержение этих мифов:\n${seoData.antifakeMarkers!.map((m) => `  — ${m}`).join('\n')}\nФормат: утверждение + факт с источником и датой, своими словами. Без формул «На самом деле — …» и «Миф о том, что … неверен» — приёмка их отклоняет. Это контент, который AI-поисковик не синтезирует без цитирования нашей страницы.`
      : '',
    (seoData.mandatoryBigrams || []).length
      ? `БИГРАММЫ GSC (1.4i): обязательно используй эти биграммы из реальных запросов пользователей — в заголовках H2 или первых предложениях блоков: «${seoData.mandatoryBigrams!.join('», «')}».`
      : '',
  ]
    .filter(Boolean)
    .join('\n\n')

  // Источник правды по правилам — ~/.claude/skills/dpub-content-standard/SKILL.md (v6.6).
  // При изменении скилла синхронизировать промпты: outline (ШАГ 2), baseWriterPrompt, редактор, SEO-ревью.
  const voiceSample = loadVoiceSample()
...
КРЮЧОК (открытие статьи): первые 2–3 предложения — не определение профессии, а конкретная точка входа: число, факт, наблюдение, которое удивляет читателя. Слова ключа "${topic.keyword}" должны быть в первых 60 словах в любой грамматической форме — но не обязательно в первом предложении. Крючок стоит сразу после H1 и ДО первого H2, без своего подзаголовка — это абзац, а не раздел.
  ...
  — «Средний таргетолог не знает, сколько стоит его время. Не в смысле "мало просит" — в смысле буквально не считал бюджет в управлении.»
  — «Половина фрилансеров-таргетологов зарабатывают ниже медианы штатного middle — по данным крупных job-платформ. Об этом не говорят на курсах.»

  Примеры как НЕ НАДО:
  — «Таргетолог — специалист по настройке рекламы в социальных сетях.» (определение)
  — «В этой статье мы разберём зарплаты таргетологов.» (анонс)
  — «Таргетолог — один из видов digital-специалистов, которых hh.ru насчитывает более 8 000 активных вакансий.» (SEO-шаблон)
...
BERT-ЯНДЕКС (4.1b): слова ключа "${topic.keyword}" — в первых 60 словах статьи, в первом H2 и в одном ответе FAQ. Не синонимами, но в любой грамматической форме и порядке слов: «образец резюме без опыта» вместо поисковой строки «резюме без опыта работы образец». Дословная форма ключа — только в title и meta description. ЗАПРЕЩЕНО: выделять ключ жирным (**ключ**) и открывать статью конструкцией «<ключ> — это …» — так выглядит SEO-текст, а не статья. Приёмка отклонит и то, и другое.
...
ЖИВОЙ ГОЛОС: пиши как в образце ниже — по ритму и конкретике, не по фразам. Ритм неровный: длинные и короткие предложения чередуются по смыслу, а не по норме. Абзац из одного предложения — только если это действительно вывод, не чаще одного на статью. Позиция автора — там, где есть основание, без формулы «миф … неверен». После «например» — конкретное имя, число или ситуация. От лица редакции — только конкретный факт из базы вакансий d-pub, без шаблонной фразы «мы разбираем тысячи вакансий». Запрещённые шаблоны приёмка отклонит: «Мы разбираем тысячи вакансий», «Миф о том, что», «на самом деле» больше одного раза.
...
- Упомяни Диджитал Паб как источник вакансий 1-2 раза органично со ссылкой [Диджитал Паб](/vacancies) — используй именно текст «Диджитал Паб» как анкор ссылки, НЕ «d-pub.ru». В таблицах без ссылки — просто «Диджитал Паб».
...
- ИСТОЧНИКИ ТЕКСТОМ: упоминай hh.ru, SuperJob, Росстат только как текст — «по данным hh.ru», «согласно SuperJob». НЕ делай кликабельных гиперссылок на внешние job-платформы — это конкуренты, трафик и SEO-вес остаются у нас.
...
DECISION SUPPORT (5.1h): перед FAQ обязательно добавь два H2: «Кому эта профессия не подойдёт» (маркированный список 3-5 сценариев с цифрами) + «Таблица сценариев: кому какой путь выбрать» (таблица 5-7 строк: Ситуация | Рекомендация | Почему).

PLURAL BRIDGE (9.9b): в тексте статьи (не обязательно в первых 60 словах) органично упомяни профессию и в единственном, и во множественном числе — например, в одном H2 говори об «аналитике данных», в другом об «аналитиках данных». Без шаблонных конструкций.
...
Markdown: ## для H2, ### для H3, **жирный** (для акцентов, никогда для ключа), таблицы, маркированные списки.`
...
Как выглядит: открывается одним конкретным фактом или наблюдением, которое читатель не ожидал — имя, число, случай, парадокс. Дальше текст разворачивает смысл этого факта. В середине — поворот: «оказывается», «но выяснилось», «ключевой момент». Финал короткий, закрывающий, без воды.
Примеры открытий в стиле направления: «В Telegram Ads не хватает людей — специалисты зарабатывают на 40% больше аналогов в ВКонтакте.» / «Больше половины фрилансеров-таргетологов зарабатывают ниже медианы штатного middle. Об этом не говорят на курсах.»
Как НЕ выглядит: определение профессии в первом предложении, нейтральный перечень фактов без поворота, вывод «таким образом» в конце.`,
      },
    ]
...
6. ГОЛОС: возьми голос лучшего драфта и сохрани его. Не добавляй ритмических ударов, «это миф», «на самом деле» и абзацев из одного предложения по норме — только там, где это реальный вывод. Шаблоны «Мы разбираем тысячи вакансий», «Миф о том, что … неверен» — убрать, приёмка их отклоняет.
7. КОНКРЕТИКА: найди все «например, [общее слово]» и замени на конкретное имя компании, число или ситуацию. «Например, IT-компании» → «например, Яндекс или небольшой продуктовый стартап».
8. ПОВОРОТ: если в черновиках есть факт, который переворачивает ожидание читателя, — оставь его на видном месте. Не выдумывай поворот ради поворота.
9. РОБОТНЫЕ ПЕРЕХОДЫ: найди и удали «подводя итог», «перейдём к», «как уже было отмечено», «в данном разделе», «рассмотрим», «не менее важно». Начни абзац сразу с содержательного факта.
10. AXES (1.3): покрыты ≥6 разных осей темы в разных H2? Нет двух H2 с одним интентом? Если нет — добавь недостающую ось.
11. ENTITY COVERAGE (3.12): покрыты классы 1 (что включает профессия), 4 (конкретные числа), 9 (алгоритм входа) из 11? Если нет — добавь. Не добавляй поверхностные блоки ради охвата — лучше меньше тем, но глубже.
12. Объём финального текста: 2000–2500 слов
...
12. РОБОТНОСТЬ: найди и замени все переходы-роботы («подводя итог», «перейдём к», «как уже отмечалось», «в данном разделе», «рассмотрим подробнее») — начни абзац сразу с факта. Ритм и длину предложений не выравнивай и не «оживляй» по норме — стиль уже выправлен.
13. AIO RESISTANCE (9.11): есть ли хотя бы один AIO-resistant элемент? (антифейк-маркер, уникальный диапазон с датой, авторская позиция с аргументом). Если нет — добавь антифейк-маркер в раздел, где разбирается стереотип, но без формулы «миф о том, что … неверен» — приёмка её отклоняет: утверждение + источник, например «обязательной сертификации не существует — по данным hh.ru».
14. MENTION CHANNELS (3.5): слова ключа "${topic.keyword}" присутствуют в title, H1, первых 60 словах, первом H2, одном ответе FAQ, meta description? В теле — в любой грамматической форме, дословно только в title/description. Если нет — добавь. Ключ выделен жирным или статья открывается «<ключ> — это …»? Сними жирный, определение убери из первого предложения в первый H2 — приёмка это отклоняет.
...
async function main() {
  ...
  // Сбой Telegram на этом этапе не должен ронять процесс: статья уже опубликована.
  let notified = false
  for (let attempt = 1; attempt <= 3 && !notified; attempt++) {
    try {
      ...
      console.error(
        `[writer] Отбойник в Telegram не отправлен (попытка ${attempt}/3): ${(e as Error).message}`
      )

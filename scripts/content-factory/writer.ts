...
Выдай JSON (без лишнего текста):
{
  ...
  const dynamicSeoBlock = [
    ...
    (seoData.antifakeMarkers || []).length
      ? `АНТИФЕЙК (AIO-resistant, 2.6c): обязательно включи в тело статьи опровержение этих мифов:\n${seoData.antifakeMarkers!.map((m) => `  — ${m}`).join('\n')}\nФормат: «На самом деле — [факт с источником и датой].» Это контент, который AI-поисковик не синтезирует без цитирования нашей страницы.`
      : '',
...
ЖИВОЙ ГОЛОС (анти-роботные паттерны):
  ...
  — ГОЛОС РЕДАКЦИИ: один раз добавь фразу от лица Диджитал Паб: «Мы разбираем тысячи вакансий каждую неделю и замечаем: [конкретное наблюдение по теме статьи].» Вставь органично в блок о рынке или требованиях.
...
9. ПОВОРОТ: есть ли в тексте момент где ожидание читателя переворачивается — «оказывается», «но выяснилось», «на самом деле»? Если нет — найди лучший факт из черновиков и добавь как поворот.
10. АСИММЕТРИЯ: добавь 1 абзац из одного предложения как ритмический акцент — неожиданный факт или вывод.
11. РОБОТНЫЕ ПЕРЕХОДЫ: найди и удали «подводя итог», «перейдём к», «как уже было отмечено», «в данном разделе», «рассмотрим», «не менее важно». Начни абзац сразу с содержательного факта.
...
13. AIO RESISTANCE (9.11): есть ли хотя бы один AIO-resistant элемент? (антифейк-маркер «такой-то миф неверен — по данным X», уникальный диапазон с датой, авторская позиция с аргументом). Если нет — добавь антифейк-маркер в раздел где разбирается стереотип.

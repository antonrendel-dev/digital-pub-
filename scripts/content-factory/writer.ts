...
import { restorePlanHeadings } from './lib/plan-headings.js'

// Сухой прогон: вся текстовая часть завода без единой записи наружу.
// Проверять правки промптов на живом кроне — значит чинить прод утром.
const DRY_RUN = process.env.FACTORY_DRY_RUN === '1'
...
interface ArticleResult {
  ...
  /** ТЗ, по которому статью принимали: сводке dry-run нужен исходник проверки. */
  tz: TechSpec
  /** Типы H2 из плана, по порядку. Пустая строка — планировщик тип не проставил. */
  h2Types: string[]
  specWarning?: string
...
/**
 * Вернуть заголовки H2 из плана после шага, который правил текст моделью.
 * Расхождение по числу H2 не чиним — печатаем оба списка, чтобы оно не молчало.
 */
function keepPlanHeadings(markdown: string, planTitles: string[], stage: string): string {
  const result = restorePlanHeadings(markdown, planTitles)
  if (result.mismatch) {
    console.warn(
      `[writer] ${stage}: H2 не сходятся с планом (${result.articleTitles.length} против ${planTitles.length}), ` +
        `заголовки не трогаю\n  план:   ${planTitles.join(' | ')}\n  статья: ${result.articleTitles.join(' | ')}`
    )
    return markdown
  }
  if (result.restored > 0) {
    console.log(
      `[writer] ${stage}: заголовков H2 восстановлено по плану ${result.restored} из ${planTitles.length - 1}`
    )
  }
...
- mandatoryBigrams: 3 топ-биграммы из поисковых запросов по теме (по Wordstat), которые должны органично войти в H2-заголовки или первые предложения H2-блоков.`,
  ...
  const outline = await askClaude(
    buildPlannerPrompt({
      topic: { title: topic.title, keyword: topic.keyword, type: topic.type },
      outlineHint,
      seoData,
    }),
    'writer'
  ...
  const plan = JSON.parse(outlineMatch[0]) as OutlinePlan
  ...
  const planText = renderPlanText(plan.h2s)
  const planHeadings = plan.h2s.map((h) => h.title)
  ...
  // Источник правды по правилам — ~/.claude/skills/dpub-content-standard/SKILL.md.
  // Промпты планировщика и писателя живут в lib/prompts.ts; при изменении скилла
  // синхронизировать их вместе с промптами редактора и SEO-ревью ниже.
  const baseWriterPrompt = buildWriterPrompt({
    keyword: topic.keyword,

    tzText,
    planText,
    lsi: seoData.lsi,
    dataGaps: seoData.dataGaps,
    successCriteria: seoData.successCriteria,
    voiceSample: loadVoiceSample(),
    dynamicSeoBlock,
  })
...
3. Сшей в один связный текст, сохранив структуру H2 из аутлайна. Заголовки H2 из плана не переформулируй: они согласованы, в них стоят ключ и биграммы. Правь текст блоков, заголовки оставляй как есть
...
Верни ТОЛЬКО финальный Markdown — без пояснений и комментариев.`,
  ...
  markdown = keepPlanHeadings(markdown, planHeadings, 'Шаг 3б')
...
12. РОБОТНОСТЬ: найди и замени все переходы-роботы («подводя итог», «перейдём к», «как уже отмечалось», «в данном разделе», «рассмотрим подробнее») — начни абзац сразу с факта. СТРОГО ЗАПРЕЩЁН шаблон «X — один из видов Y, которых...»: это забаненная SEO-конструкция, найдёшь — перепиши предложение. Ритм и длину предложений не выравнивай и не «оживляй» по норме — стиль уже выправлен.
...
14. MENTION CHANNELS (3.5): заголовки H2 из плана не переформулируй — они согласованы, в них стоят ключ и биграммы; правь текст блоков, заголовки оставляй как есть. Слова ключа "${topic.keyword}" присутствуют в title, H1, первых 60 словах, первом H2, одном ответе FAQ, meta description? В теле — в любой грамматической форме, дословно только в title/description. Если нет — добавь. Ключ выделен жирным или статья открывается «<ключ> — это …»? Сними жирный, определение убери из первого предложения в первый H2 — приёмка это отклоняет.
...
СТРОГО ЗАПРЕЩЕНО после статьи добавлять: сводку правок, таблицу изменений, чеклист выполненных задач, комментарии вида «Что изменено», «Сводка», «Задача / Статус / Правка» и любой другой служебный текст. Ответ заканчивается последней строкой статьи — и ничем больше.`,
  ...
  const reviewedFinal = keepPlanHeadings(
    reviewedWords >= preReviewWords * 0.6 ? reviewedCandidate : markdown,
    planHeadings,
    'Шаг 4'
  )
  ...
  return {
    markdown: stripHtmlComments(accepted.markdown),
...
/**
 * Сводка сухого прогона: всё, ради чего он и запускается — что модель написала,
 * прошло ли это механическую приёмку и куда смотреть, если нет. Пишет один файл
 * рядом со стенограммой прогона и ничего больше: ни content/articles, ни
 * topics_*.json сухой прогон не трогает.
 */
function reportDryRun(topic: Topic, result: ArticleResult, runDir: string | null): void {
  const publishedAt = new Date().toISOString().split('T')[0]
  const body = normalizeFaqHeading(stripServiceTail(result.markdown))
  // Картинок в сухом прогоне нет, поэтому imageUrl = null и injectImages не зовём:
  // проверяем ровно тот текст, который написали агенты.
  const mdxContent = buildMdxFrontmatter(topic, result, publishedAt, null, body) + '\n' + body

  // Тот же гейт, что и на живом прогоне: выражения и опасные теги в MDX ловятся
  // здесь, а не после публикации.
  assertSafeMdx(mdxContent, `writer ${result.slug} (dry-run)`)

  let mdxPath = 'не сохранён (каталог прогона не открылся)'
  if (runDir) {
    mdxPath = path.join(runDir, 'dry-run.mdx')
    fs.writeFileSync(mdxPath, mdxContent)
  }

  const words = body.split(/\s+/).filter(Boolean).length
  const h2s = body.split('\n').filter((l) => l.startsWith('## '))
  const violations = checkTechSpec(result.tz, body)
  const robot = scoreArticle(body)
  ...
  console.log(
    [
      ...
      '─── DRY-RUN: сводка ─────────────────────────────────────────',
      `slug: ${result.slug}`,
      `metaTitle (${result.metaTitle.length} симв): ${result.metaTitle}`,
      `metaDesc (${result.metaDesc.length} симв): ${result.metaDesc}`,
      `Тело статьи: ${words} слов`,
      `H2: ${h2s.length}, типы по плану: ${result.h2Types.map((t) => t || '—').join(', ') || 'не проставлены'}`,
      'checkTechSpec: ' +
        (violations.length
          ...
          : 'чисто'),
      'Роботность: ' +
        (robot.violations.length ? `\n${formatViolations(robot.violations)}` : 'чисто'),
      `MDX: ${mdxPath}`,
      `Стенограмма: ${runDir ?? 'не велась'}`,
      '─────────────────────────────────────────────────────────────',
    ].join('\n')
  )
}
...
async function main() {
  ...
  if (DRY_RUN) console.log('[writer] РЕЖИМ DRY-RUN: ничего не публикуется')
  ...
  // Дальше начинаются побочные эффекты: картинки, запись MDX, коммит, прод,
  // анонс. Сухой прогон заканчивается здесь — вся текстовая часть уже пройдена.
  if (DRY_RUN) {
    reportDryRun(topic, result, runDir)
    return
  }

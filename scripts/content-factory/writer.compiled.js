...
async function generateImageWithCodex(imagePrompt, slug) {
  ...
  const fullPrompt = `Generate a hero image for a blog article using this exact style: High-fidelity pixel art illustration in the style of modern indie games (Stardew Valley, Octopath Traveler), fine pixel detail with small pixel grid, rich color depth, smooth gradients via dithering, warm desk-lamp or ambient lighting, soft amber glow, muted warm color palette with dusk blue shadows, charming isometric or side-view composition, nostalgic indie game atmosphere, no photorealism, no watermark, no text in image. SCENE: ${imagePrompt}. Use your image generation tool to create this image now.`
  console.log(
